...
public class GradientButton : Button
{
    ...
    protected override void DoStateTransition(UnityEngine.UI.Selectable.SelectionState state, bool instant)
    {
        ...
        if(state <= 4)
        {
                var val_9 = 32498016 + (state) << 2;
            val_9 = val_9 + 32498016;
        }
        else
        {
                ???.CrossfadeGradient(gradient:  ??? + 312, newGradientInfo:  null);
    ...
    private void CrossfadeGradient(UIGradient gradient, GradientInfo newGradientInfo)
    {
                ...
                if(this.routine != null)
        {
            ...
            this.routine = this.StartCoroutine(routine:  this.DoCrossfadeGradient(gradient:  gradient, newGradientInfo:  newGradientInfo = newGradientInfo));
            return;
        }
    ...
    private System.Collections.IEnumerator DoCrossfadeGradient(UIGradient gradient, GradientInfo newGradientInfo)
    {
        ...
        return (System.Collections.IEnumerator)new GradientButton.<DoCrossfadeGradient>d__10();
    }
